defmodule JeopardyWeb.Components.Contestant.ReadingFinalJeopardyClue do
  ...
  def assign_init(socket, game) do
    ...
    {:ok,
     assign(socket,
       form: %{answer: contestant.final_jeopardy_answer},
    ...
    <div>
      <div :if={not @has_submitted_answer?}>
        <.form for={@form} phx-change="validate" phx-submit="submit" phx-target={@myself}>
          <.input type="number" field={@form[:answer]} placeholder="Answer" />
  ...
  def handle_event("submit", %{"answer" => answer}, socket) do
    with {:ok, _game} <- GameServer.action(socket.assigns.code, :answered, answer) do
